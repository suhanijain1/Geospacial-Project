...
A thread is a unique ID or thread identifier assigned to each checkpoint saved by a checkpointer. It contains the accumulated state of a sequence of [runs](../assistants/#execution). When a run is executed, the [state](../low_level/#state) of the underlying graph of the assistant will be persisted to the thread.
...
### Semantic Search[¶](#semantic-search "Permanent link")
...
```
### Using in LangGraph[¶](#using-in-langgraph "Permanent link")
...
#### Serialization with `pickle`[¶](#serialization-with-pickle "Permanent link")
...
```
#### Encryption[¶](#encryption "Permanent link")
