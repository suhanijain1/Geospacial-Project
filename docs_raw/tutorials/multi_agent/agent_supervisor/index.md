...
# Multi-agent supervisor[¶](#multi-agent-supervisor "Permanent link")
...
## 1. Create worker agents[¶](#1-create-worker-agents "Permanent link")
...
### Research agent[¶](#research-agent "Permanent link")
...
```
### Math agent[¶](#math-agent "Permanent link")
...
56
```
## 2. Create supervisor with `langgraph-supervisor`[¶](#2-create-supervisor-with-langgraph-supervisor "Permanent link")
...
*API Reference: [create\_supervisor](https://langchain-ai.github.io/langgraph/reference/supervisor/#langgraph_supervisor.supervisor.create_supervisor) | [init\_chat\_model](https://python.langchain.com/api_reference/langchain/chat_models/langchain.chat_models.base.init_chat_model.html)*
```
...
```
## 3. Create supervisor from scratch[¶](#3-create-supervisor-from-scratch "Permanent link")
...
### Set up agent communication[¶](#set-up-agent-communication "Permanent link")
...
### Create supervisor agent[¶](#create-supervisor-agent "Permanent link")
...
```
### Create multi-agent graph[¶](#create-multi-agent-graph "Permanent link")
...
* **how agent outputs are added**: you can control how much of the agent's internal message history is added to the overall supervisor message history by wrapping the agent in a separate node function:
  ...
  ```
## 4. Create delegation tasks[¶](#4-create-delegation-tasks "Permanent link")
